...
FREE_LIMIT = 5  # лимит бесплатных запросов для гостей
...
def register(request: RegisterRequest, db: Session = Depends(get_db)):
    ...
    user = User(email=request.email, hashed_password=hashed, balance=5)  # 5 приветственных запросов
    db.add(user)
    db.commit()
    db.refresh(user)
...
def get_remaining(request: Request, db: Session = Depends(get_db),
    ...
    - Если пользователь авторизован — отдаёт balance.
    - Если гость — считает по session_id из куки.
    """
    if user:
...
class TopUpRequest(BaseModel):
    amount: int = 1  # количество запросов для добавления
...
def top_up_balance(request: TopUpRequest, db: Session = Depends(get_db),
                   user: User = Depends(get_current_user)):
    """Заглушка пополнения баланса (пока без оплаты)."""
    if not user:
